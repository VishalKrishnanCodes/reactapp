...
// Sample student data imported from StudentProfile component
const studentData = {
  '2023-2024': [
...
export default function Landing({ username, onLogout }) {
  const years = ['2023-2024', '2024-2025', '2025-2026', '2026-2027'];
  const [selected, setSelected] = useState(years[0]);
  const [selectedStudentId, setSelectedStudentId] = useState(null);

  const students = studentData[selected] || [];
  ...
  return (
    <div className="landing-container">
      ...
      <div className="table-container">
        <h3>Students - Academic Year {selected} ({students.length} students)</h3>
        {students.length > 0 ? (
          <table className="students-table">
            ...
            <tbody>
              {students.map((student) => (
                <tr key={student.id} onClick={() => setSelectedStudentId(student.id)} className="clickable-row">
                  <td>{student.id}</td>
                  <td>{student.name}</td>
                  <td>{student.dob}</td>
